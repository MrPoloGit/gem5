...
class L2Cache(Cache):
    # Default size, can be overwritten by args
    size = '256kB'
    assoc = 8
    tag_latency = 20
    data_latency = 20
    ...
    def __init__(self, size_override=None):
        super().__init__()
        if size_override:
...
parser.add_argument('--l2-size', type=str, default='256kB',
                    help='Size of the L2 Cache (default: 256kB)')
parser.add_argument('--mem-size', type=str, default='512MB',
                    help='Size of the Main Memory (default: 512MB)')
...
# Set up memory mode and ranges (Dynamic from args)
...
# Create the L2 Cache (Dynamic size from args)
system.l2cache = L2Cache(size_override=args.l2_size)
...
print(f"    Mem Size: {args.mem_size}")

if args.prefetcher == 'mlop':
    print(f"    MLOP Lookahead: {args.mlop_lookahead}")
    print(f"    MLOP Eval Period: {args.mlop_eval_period}")
    print(f"    MLOP Score Thresh: {args.mlop_score_threshold}")
