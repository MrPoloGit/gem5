...
mlop_group.add_argument('--mlop-eval-period', type=int, default=500,
                        help='Number of accesses per scoring epoch (default: 500)')
mlop_group.add_argument('--mlop-lookahead', type=int, default=16,
                        help='Number of lookahead levels (default: 16)')
mlop_group.add_argument('--mlop-max-offset', type=int, default=63,
                        help='Max signed offset in cache lines (default: 63)')
mlop_group.add_argument('--mlop-score-threshold', type=int, default=200,
                        help='Score threshold for offset selection (default: 200)')
mlop_group.add_argument('--mlop-amt-entries', type=int, default=256,
                        help='Number of entries in the Address Map Table (default: 256)')
mlop_group.add_argument('--mlop-bit-vector-size', type=int, default=64,
                        help='Size of the bit-vector (default: 64)')
...
if args.prefetcher == 'bingo':
    ...
    prefetcher.amt_entries = args.mlop_amt_entries
    prefetcher.bit_vector_size = args.mlop_bit_vector_size
    
    # MLOP uses a specific 'prefetch_degree' parameter in its python definition
    # rather than the base QueuedPrefetcher 'degree' for its calculation logic,
    # though it might inherit from Queued. We set the specific param here.
    if args.degree > 0:
