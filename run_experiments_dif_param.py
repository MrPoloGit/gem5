...
experiments = [
    # 1. Lookahead Depth Sensitivity
    # Tests if looking further ahead captures better patterns.
    # Vary: --mlop-lookahead
    {
        "name": "mlop_lookahead",
        "vary_param": "--mlop-lookahead",
        "values": ["4", "8", "16", "32"],
        # Default flags for this experiment
        "flags": [
            "--prefetcher", "mlop",
            "--l2-size", "256kB",
            "--mem-size", "512MB"
    ...
    # 2. Training Window Adaptation
    # Tests responsiveness to phase changes vs. statistical stability.
    # Vary: --mlop-eval-period
    {
        "name": "mlop_window",
        "vary_param": "--mlop-eval-period",
    ...
    # Note: We handle this loop manually in the task builder below because it requires changing 2 flags.
    {
        ...
        "configs": [
            {"label": "constrained", "l2": "128kB", "mem": "512MB"},
            {"label": "baseline",    "l2": "256kB", "mem": "512MB"},
            {"label": "abundant",    "l2": "1MB",   "mem": "512MB"}
        ],
        "flags": ["--prefetcher", "mlop"]
    },

    # 4. Confidence vs. Coverage
    # Tests filtering weak patterns to save bandwidth.
    # Vary: --mlop-score-threshold
    {
        "name": "mlop_confidence",
        "vary_param": "--mlop-score-threshold",
        "values": ["0", "10", "50"],
        "flags": [
            "--prefetcher", "mlop",
            "--l2-size", "256kB",
