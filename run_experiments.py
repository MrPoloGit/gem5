...
prefetchers = ["none", "bingo", "stride", "mlop", "bop"]
workloads = ["bt", "cg", "dc", "ft", "is", "lu", "mg", "sp"]


def run_pool():
    # Creates a queue of all tasks (flatten the loops)
    tasks = [(pref, wl) for pref in prefetchers for wl in workloads]

    total_tasks = len(tasks)
    print(f"==================================================")
    print(
        f"Queued {total_tasks} experiments. Max concurrent: {MAX_CONCURRENT}"
    )
    print(f"==================================================")
    ...
    # Main Loop: continue while there are tasks left to start OR tasks currently running
    while tasks or running:
        ...
        for i in range(len(running) - 1, -1, -1):
            ...
            if ret is not None:
                ...
                if ret == 0:
                    print(
                        f"  [Success] {pref}/{wl} finished. ({completed_count}/{total_tasks})"
                    )
                else:
                    print(
                        f"  [FAILED]  {pref}/{wl} exited with code {ret}. Check logs."
                    )

                # Remove from running list
                del running[i]

        # Fill empty slots if we have tasks waiting
        ...
        # Wait a bit before checking again to prevent CPU spinning
        if running:
            time.sleep(1)
